<script lang="ts">
    ...
    import {library, type Track, type Current, type State, type Library, type Duration} from "$lib/state/state.svelte";
   ...
   $effect(() => {
        ...
        if (library.position) {
            if (library.current) {
                if (library.position.secs == library.current.song.len?.secs) {
                    ...
                    invoke("next", {dispatcher}).then(_res  => {}).catch(_err => {})
                }
            }
        }
    })
    ...
    $effect(() => {
        ...
        if (library.current && !library.current.paused) {
            ...
            invoke("position", {dispatcher}).then(_res  => {
                }).catch(_err => {})
            }, 1000)
        }
    return () => {
    ...
    invoke("next", {dispatcher}).then(res  => console.log(res)).catch(err => console.log(err))
    }
    ...
    invoke("back", {dispatcher}).then(res  => console.log(res)).catch(err => console.log(err))
    }
    ...
    invoke("pause", {dispatcher}).then(res  => console.log(res)).catch(err => console.log(err))
    }
    ...
    invoke("play", {dispatcher}).then(res  => console.log(res)).catch(err => console.log(err))
    }
...
async function seek(dur: number) {
    ...
    invoke("seek", {dur, dispatcher}).then(res  => console.log(res)).catch(err => console.log(err))
    }
...
<main>
    ...
    <section id="content">
        <article>
                <div>
                    ...
                    <button aria-labelledby="actions button" onclick={() => {
    ...
    <section id="controls">
            <nav>
                ...
                <li onclick={back}>
<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512"><path d="M112 64a16 16 0 0 1 16 16v136.43L360.77 77.11a35.13 35.13 0 0 1 35.77-.44c12 6.8 19.46 20 19.46 34.33v290c0 14.37-7.46 27.53-19.46 34.33a35.14 35.14 0 0 1-35.77-.45L128 295.57V432a16 16 0 0 1-32 0V80a16 16 0 0 1 16-16Z"/></svg></li>
                <li>
                   {#if !library.current || library.current?.paused}
                <!-- svelte-ignore a11y_click_events_have_key_events -->
                <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
<!-- svelte-ignore a11y_no_static_element_interactions -->
            ...
            <div id="outer">
                <div id="inner"
                        style="width:{
                        library.ui.positionPercent(library.position.secs,
...
<li onclick={musicWindow}>
            ...
            <li>
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M2 18h10v2H2v-2Zm0-7h14v2H2v-2Zm0-7h20v2H2V4Zm17 11.17V9h5v2h-3v7a3 3 0 1 1-2-2.83Z"/></svg>
                <span>Playlist</span>
            </li>
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
            <li onclick={load}>
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12.414 5H21a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h7.414l2 2ZM11 13.05a2.5 2.5 0 1 0 2 2.45V11h3V9h-5v4.05Z"/></svg>
                <span>Browse</span>
            </li>
        </ul>
    </nav>
...
<style>
    ...
    main nav#actions ul li:hover, main nav#actions ul li:hover svg{
   ...
   main img {
    ...
    main section#content article p#song_name {
        min-width: 195px;
        max-width: 195px;
        margin-block: 5px;
    ...
    main section#content article::after {
        content: '';
        position: absolute ;
        width: 40%;
        height: 0.5px;
...
main section#content article div {
        display: flex;
        position: relative;
...
main section#content article div button {
        display: none;
        position: absolute;
        top: 5px;
        right: -17px;
...
main section#content:hover article div button {
        display: block;
    }
...
main #controls nav ul li {
        width: 20px;
    }

main #controls nav ul li svg{
        width: 100%;
        height: 100%;
        fill: var(--text-color);
    }

main #controls nav ul li svg:hover{
        fill: var(--hover-color);
    }

main #controls nav ul li svg:active{
        fill: var(--active-color);
    }

main #controls nav #outer{
...
main #controls nav #inner {
        height: 100%;
        border-radius: var(--border-radius, 10px);
        background-color: var(--hover-color);
    }
main #controls nav #time {
        display: flex;
...
main #controls nav:hover #position {
        display: block;
    }
